...
export async function PUT(_: Request, { params }: { params: { id: string } }) {
  ...
  try {
    // For skip, we mark the habit as skipped for today
    // We could create a separate "skipped" table, but for now we'll just return the habit
    // without adding a completion record
    ...
    return NextResponse.json({ error: 'Failed to skip habit' }, { status: 500 })
  }
}
